...
#### Model parameters $\phi$, $\theta$ and $\sigma^2$
 For estimation of the model parameters the maximum likelihood function is used. We assume that the model order is known, and that the data has zero mean. From earlier we assumed the model 
 
\[
 X_t =\phi_1X_{t-1} - \phi_2X_{t-2} + Z_t + \theta_1Z_{t-1}.
\]
The $\{X_t\}$ is gaussian distributed since the white noise included in the model is assumed to be normal distributed and independent of each other, $\{Z_t\} \sim N(0,\sigma^2)$. The approach for estimating to parameters is then to maximize a log-likelihood function that follows a gaussian distribution. This optimization is done numerically. The likelihood function is defined as 
