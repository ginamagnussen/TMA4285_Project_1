...
# Abstract
In this exercise we analyse an ARMA(2,1) time series. The series is first explored using its sample autocorrelation function (ACF), its partial ACF, and with stationarity checks. We then estimate the ARMA(2,1) coefficients and predict values for both existing and future data points, and checking the model assumtions by inspecting the residuals. Finally, in diagnosis of the model the choice of model order is examined.

# Introduction
Analysis on time series data has multiple objectives. Representing a time series as an ARMA-model we are able to describe data in a very compact an efficient way. Furthermore we can easier interpret the series (given that the ARMA actually model the reality), since we can distinguish noise from signal say more about the implications for the "outside" noise and the "inherent" fluctuations. It is also possible to do forecasting by generating new realizations ("innovations") of the chosen model, which of course is subject to future random noise and uncertianty in the model coefficients.
