...
# Introduction
Analysis of time series data has multiple objectives. Representing a time series as an ARMA-model we are able to describe data in a very compact way. Furthermore it is easier to interpret the series (given that the model fits the reality reasonably good), since we can distinguish noise from signal and understand more about the implications for the "outside" noise and the "inherent" fluctuations. Forecasting can be done by generating new realizations ("innovations") of the chosen model, which of course is subject to future random noise and uncertianty in the model coefficients.


# Theory
## What is a time series and an ARMA-process.
 
A time series is a sequence of observations with timestamps. If there is some random noise involved in the data, we might represent the time series as an ARMA model. That means that the data value at a given time $t$ is a linear combination of the last $p$ data realizations added to a linear combination of the last $q$ white-noise terms $Z$. An ARMA(2,1)-process is therefore a model that includes the last two datapoints (at least explicitly), and the last noise-term. Using the backshift operator $B$ for short hand notation, this is expressed as
 \[
...
The partial autocorrelation function is the correlation function in which the linear effects of $\{x_{t+1},x_{t+2},...,x_{t-1+h}\}$ (all realizations in between) are removed. That means that the only contribution to the correlation between two values at lag $h$ is the direct correlation between the two, and not the implicit correlation from one $x$ to the next. For example will an AR(1)-process have implications for realizations far beyond one time step, but removing this recursive feature we can use the PACF for deciding on the appropriate order $p$. 
...
ACF: There is most correlation at lag $h=1,2$ and $3$, and thus these values could be appropriate choices for $q$.

PACF: From the partial correlation plot, we see that lag $h=1$ could be a good choice for $p$. 
 ...
 * Finally, comment on the diagnostics, AIC
...
This plot also indicates that the fitted model is a good choice.
