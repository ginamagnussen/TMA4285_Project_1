...
#### Calculation of ACF
After establishing the ARMA-model, we want to plot the sample autocorrelation function (ACF). This will reveal if the data is likely to origin from an actual ARMA(2,1), or if behaves significantly different from expected. The sample autocovariance function (ACVF) is defined as

\[
\hat{\gamma} = n^{-1}\sum_{t=1}^{n-|h|}(X_{t+|h|}-\bar{X}_n)(X_{t}-\bar{X}_n),
\]
and we obtain the ACF be normalizing: $\hat{\rho}(h) = \frac{\hat{\gamma}(h)}{\hat{\gamma}(0)}$.
